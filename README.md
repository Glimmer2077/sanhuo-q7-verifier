...
- Q3 不只接受“范围正确”的摘要：三个 H1-A screen 必须分别绑定父候选
  调度、20 秒前缀哈希、screen 调度哈希和约 17.1 秒的已知反向点；Q4
  固定核对 arm/H0/ACK 首错停止和第二次 arm 拒绝。
- Q5 不接受 Python 自行构造的安全统计。目标仓库的 host harness 必须让
  每个候选真实执行 200 次正常 + 200 次故障回调，并绑定 core、共享 screen
  adapter、实际 schedule、harness、可执行文件和 stdout。可信外层同时用
  本仓库内置、不会 include 目标 harness 的 C++ 程序重新编译并执行同一
  core/adapter，交叉核对两层均为故障后性能写入 0、最多一次安全回中，
  并覆盖最终安全回中自身失败。
