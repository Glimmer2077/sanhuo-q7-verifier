...
- 四候选的 `build / qualify / audit` 共 12 个固定命令，每个命令使用新的
  ...
  继承的生命周期标记；可信外层在封存前按 PID + 启动时间查找、强制终止
  并排空全部残留标记进程，发现任何残留即判该步失败。沙箱内再次套用新
  profile 以隐藏标记会被 macOS 拒绝，并有真实对抗测试覆盖。因此旧后台
  进程不能在封存时继续持有当前步骤的写权限。
