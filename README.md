...
`phase2c-h1a` profile、三个候选
`MF-T0-H1A / MF-T1-H1A / MF-T2-H1A` 生成和验证离线 Q7 双审查证据。
...
- 同时只复制构建源码差异审计所需的固定冻结基线
  `8ae75f9a4082094784ac4b8f466d1466dd5ab5f2` 和 Phase 2C 安全门基线
  `e9b8675944742cb729883ca767f5a5a98b773954`，不复制任意额外引用。
...
- 三候选的 `build / qualify / audit` 共 9 个固定命令，每个命令使用新的
...
- ESP-IDF 不再直接使用可能含忽略文件的本机 worktree；外层从锁定提交和
  递归子模块 Git 对象重建只读快照，逐 blob 核对身份，并在 9 个动作结束
  后复核完整闭包。
- 当前 ELF 的原始哈希、去符号语义哈希和能力，在最终只读快照上由本仓库
  代码重新计算。Q0～Q6 七份原始报告也由可信层重算哈希并逐门验收安全语义，
  不接受被审代码自己声明的 `passed` 摘要代替原始事实。
- Q2 固定要求 Phase 2C 锁定测试文件完整收集、执行并通过 13 项测试，不接受
  跳过、预期失败、意外通过或只修改摘要数字。
- Q3 不只接受“范围正确”的摘要：三个 H1-A screen 必须公开完整的 434、
  364、206 条规范化命令。可信层逐字段检查时间、轴、位置、时长、速度和
  来源时间，重算 20 秒前缀、完整 screen、生成头文件三层哈希，并绑定父
  候选证据、验证器自有的冻结哈希和约 17.1 秒的已知反向点；Q4 固定核对
  arm/H0/ACK 首错停止和第二次 arm 拒绝。
- Q5 不接受 Python 自行构造的安全统计。目标仓库的 host harness 必须让
  每个候选真实执行 200 次正常 + 200 次故障回调，并绑定 core、共享 screen
  adapter、实际 schedule、harness、可执行文件和 stdout。可信外层用自己
  重建的精确生成头文件编译本仓库内置、不会 include 目标 harness 的 C++
  程序，逐条执行真实 `GeneratedCommand` 字段；可信结果和身份先锁入内存，
  再在独立临时目录复建目标 harness。复建命令从目标 Phase 2C 项目根运行，
  对目标源码和固定 payload include 使用与目标 CLI 相同的仓库相对路径，
  避免检出目录进入 C++ 断言元数据；随后逐字节比较目标可执行文件与 stdout。
  两层均核对故障后性能写入 0、最多一次安全回中，并覆盖最终安全回中自身
  失败；任何目标复建错误都只会失败关闭，不能改写已捕获的可信结论。
